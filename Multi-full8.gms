...
lake_res(m)..                    lake_inflows(m)                 =g=     sum(h$(month(h) = ord(m)),GENE('lake',h))*8/1000;
stored_cap(str,h)..              SOC(str,h)*8                    =l=     CAPACITY(str);
storage_capa(str,h)..            STORAGE(str,h)                  =l=     CAPA(str);
conversion(h,conv)..             GENE(conv,h)                    =e=     CONVERT(conv,h)*eta_conv(conv);
nuc_cf..                         sum(h,GENE('nuclear',h))        =l=     CAPA('nuclear')*cf_nuc*8760/8;
*nuc_up(h,h+1)..                  GENE('nuclear',h+1) + RSV('nuclear',h+1) =l= GENE('nuclear',h) + ramp_rate*CAPA('nuclear')*8  ;
*nuc_down(h,h+1)..                GENE('nuclear',h+1) =g= GENE('nuclear',h)*(1 - ramp_rate)   ;
ccgt_cf..                        sum(h,GENE('CCGT',h))           =l=     CAPA('CCGT')*cf_ccgt*8760/8;
ccs_cf..                         sum(h,GENE('CCGT-CCS',h))       =l=     CAPA('CCGT-CCS')*cf_ccs*8760/8;
methanisation_max..              sum(h,GENE('methanization',h))  =l=     max_metha*1000/8;
pyrogasification_max..           sum(h,GENE('pyrogaseification',h))=l=   max_pyro*1000/8;
...
max_heat..                       sum(h,GENE('heat-net',h))       =l=     max_central*1000/8;
...
ev_volume(w)..                   sum((h,ev)$(ord(w)=week(h)),GENE(ev,h)) =l= EV_VOL/8;
...
obj..                            COST                            =e=     (sum(tec,(CAPA(tec)-capa_ex(tec))*capex(tec))+ sum(str,CAPACITY(str)*capex_en(str))+sum(tec,(CAPA(tec)*fOM(tec))) +sum((tec,h),GENE(tec,h)*vOM(tec)*8)+(EV_VOL*EV_batt))/1000;
*-------------------------------------------------------------------------------
...
file hourly_profiles /'outputs/hourly_profiles8_nonuc.csv'/ ;
file results_csv /'outputs/summary8_nonuc.csv'/;
parameter nSTORAGE(str,h);
parameter nCONVERT(conv,h);
put hourly_profiles;
put 'hour'; loop(tec, put tec.tl;) put 'demand_elec', 'ElecStr','Pump','demand_hydrogen','gastank','demand_heat','CTES','ITES','elec_market','gas_market','heat_market',loop(conv, put conv.tl;) put 'OK'/ ;
put results_csv;
results_csv.pc=5;
...
Solve EOLES8 using lp minimizing COST;
sumgene_elec = sum((elec_gen,h),GENE.l(elec_gen,h))*8/1000;
sumgene_gas = sum((gas_gen,h),GENE.l(gas_gen,h))*8/1000;
gene_tec(tec) = sum(h,GENE.l(tec,h))*8/1000;
dem_h2 = sum(h,demand_hydrogen(h))*8/1000;
dem_elec = sum(h,demand_elec(h))*8/1000;
dem_heat = sum(h,demand_heat(h))*8/1000;
dem_transport = sum((transporttype,h),demand_transport(transporttype,h))*8/1000;
elec_gas = sum(h,(CONVERT.l('methanation',h)+CONVERT.l('electrolysis',h)))*8/1000;
gas_elec = sum(h,(CONVERT.l('OCGT',h)+CONVERT.l('CCGT-CCS',h)))*8/1000;
elec_heat = sum(h,(CONVERT.l('resistive',h)+CONVERT.l('hpc',h)+CONVERT.l('hpd',h)))*8/1000;
gas_heat = sum(h,(CONVERT.l('boilerd',h)+CONVERT.l('boilerc',h)))*8/1000;
elec_transport = sum((h,ev),CHARGE.l(ev,h))*8/1000;
gas_transport = sum((h,ice),CHARGE.l(ice,h))*8/1000;
...
elec_spot_price(h) = 1000000*adequacy_elec.m(h)/8;
gas_spot_price(h) = 1000000*adequacy_gas.m(h)/8;
heat_spot_price(h) = 1000000*adequacy_heat.m(h)/8;
h2_spot_price(h) = 1000000*GENE.m('electrolysis',h)/8;
elec_marginal_cost = sum(h,elec_spot_price(h))*8/8760;
gas_marginal_cost = sum(h,gas_spot_price(h))*8/8760;
heat_marginal_cost = sum(h,heat_spot_price(h))*8/8760;
h2_marginal_cost = sum(h,h2_spot_price(h))*8/8760;
G2P_bought = sum(h,gas_spot_price(h)*(CONVERT.l('OCGT',h)+CONVERT.l('CCGT-CCS',h)))*8/1000;
P2G_bought = sum(h,elec_spot_price(h)*(CONVERT.l('methanation',h)+CONVERT.l('electrolysis',h)))*8/1000;
G2H_bought = sum(h,gas_spot_price(h)*(CONVERT.l('boilerc',h))+CONVERT.l('boilerd',h))*8/1000;
P2H_bought = sum(h,elec_spot_price(h)*(CONVERT.l('hpd',h)+CONVERT.l('hpc',h)+CONVERT.l('resistive',h)))*8/1000;
P2T_bought = sum((h,ev),elec_spot_price(h)*GENE.l(EV,h))*8/1000;
G2T_bought = sum((h,ice),gas_spot_price(h)*GENE.l(ice,h))*8/1000;
