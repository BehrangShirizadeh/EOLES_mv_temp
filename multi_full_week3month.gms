...
/
$include inputs/demand_hydrogen2050_week3month.csv
/;
parameter demand_heat(h) 'heat demand profile in each hour in GWh-th'
*resource: GRTgas + TIGF
/
$include inputs/demand_heat2050ADEME_week3month.csv
/;
parameter demand_transport(transporttype,h)    'hourly transport demand in GWh-useful'
/
$include inputs/demand_transport2050_week3month.csv
/;
...
/
$include inputs/length_3month.csv
/ ;
$offdelim
$Onlisting
...
lake_res(m)..                    lake_inflows(m)*3                 =g=     sum(h$(month(h) = ord(m)),GENE('lake',h)*coef(m)/168)/1000;
...
methanisation_max..              sum(h,GENE('methanization',h))  =l=     max_metha*1000*672/8760;
pyrogasification_max..           sum(h,GENE('pyrogaseification',h))=l=   max_pyro*1000*672/8760;
...
max_heat..                       sum(h,GENE('heat-net',h))       =l=     max_central*1000*672/8760;
...
obj..                            COST                            =e=     (sum(tec,(CAPA(tec)-capa_ex(tec))*capex(tec))+ sum(str,CAPACITY(str)*capex_en(str))+sum(tec,(CAPA(tec)*fOM(tec))) +sum((tec,h),GENE(tec,h)*vOM(tec)*8760/672)+EV_VOL*EV_batt)/1000;
*-------------------------------------------------------------------------------
...
file hourly_profiles /'outputs/hourly_profiles_rep3.csv'/ ;
file results_csv /'outputs/summary_rep3.csv'/;
...
Solve EOLES_rep3 using lp minimizing COST;
